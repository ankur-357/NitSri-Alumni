...
const SideNav = () => {
  ...
  return (
    <div className={`border-r z-20 lg:sticky md:relative transition-all top-0 fixed border-gray-800 px-3 py-8 lg:flex md:flex flex-col gap-2 items-start w-[16rem] h-full pt-24 ${show ? null : "lg:left-0 md:left-0 lg:translate-x-0 md:translate-x-0 translate-x-[-100%]"}`}>
      ...
      <div className="flex flex-col  lg:gap-1 gap-3 sm:gap-4">
        {
          links.map((link, index) => (
            ...
            >
              {({ isActive }) => (
                <button onClick={() => setShow(false)} className={`text-sky-500 w-full flex items-center gap-3 font-medium text-start px-4 py-2 rounded-xl ${isActive ? "bg-[#4B164C] text-white rounded-xl" : "hover:bg-black hover:text-white"}`}>
                  {link.icon} {link.name}
                </button>
              )}
            </NavLink>
