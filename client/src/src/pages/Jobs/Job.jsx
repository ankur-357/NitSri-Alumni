...
const Job = () => {
    ...
    try {
      ...
      if (data.status === "published") {
        await sendNotification(
          job.userID,
          "Job Published",
          `Your job "${job.jobTitle}" has been published!`
        );
      } else if (data.status === "rejected") {
        await sendNotification(
          job.userID,
          "Job Rejected",
          `Your job "${job.jobTitle}" has been rejected!`
        );
      }
      toast.success(`job ${data.status} successful!`);
      navigate(`/admin/jobs?type=${data.status}&page=1`);
    } catch (error) {
  ...
  return (
    <div className="pt-24 min-h-screen">
      <Meta name={job ? job.title : "Experience - NIT Srinagar"} />
      <Heading heading={"Review Job"} />

      <div className="lg:max-w-[85%] md:w-[90%] w-full px-5 pt-16 m-auto">
        {isPending ? (
            ...
            <div className="border border-gray-800 rounded-2xl p-5 mb-5 w-full">
                ...
                <div className="flex gap-5 flex-col items-center">
                  <div className="flex w-full gap-2 items-center">
                    <div className="flex flex-col">
                      <p className="font-semibold text-rose-500 lg:text-xl text-lg">
                        {job.jobCompany}
                      </p>
                  ...
                  <p className="text-sm text-gray-400">
                    ...
                    <span className="text-white">
                      {new Intl.DateTimeFormat("en-IN", {
                        ...
                        day: "2-digit",
                        weekday: "short",
                      }).format(new Date(job.jobDeadline))}
                    </span>
                  </p>
                </div>
              </div>
              {job.jobCompanyDescription && (
                <div className="mt-3">
                  <p className="">
              ...
              {job.jobSalary != 0 && (
                <div>
                  <p className=" text-gray-400">
                    Expected Salary:{" "}
                    <span className="text-white">{job.jobSalary} LPA</span>
                  </p>
                </div>
              )}
              {job.jobDetailsLink && (
                <div className="flex gap-2">
                  ...
                  <a
                    href={getDownloadURL(job.jobDetailsLink)}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <button className="text-sm text-sky-500">download</button>
                  </a>
                </div>
              )}
                ...
                <div>
                  ...
                  {job.jobLinks.map((link, idx) => (
                    <a key={idx} href={link} target="_blank" rel="noreferrer">
                      <button className="text-sm text-left text-sky-500">
                        {link}
                      </button>
                    </a>
                  ))}
              ...
              {job.jobCompanyEmail && (
                <div>
                  ...
                  <a
                    target="_blank"
                    href={`mailto:${job.jobCompanyEmail}`}
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-2 items-center">
                  <div className="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center">
                    <img
                      src={`https://cloud.appwrite.io/v1/avatars/initials?name=${job.name
                        .split(" ")
                        .join("+")}&width=80&height=80`}
                      alt="User Profile"
                    />
                  </div>
                  <div className="flex flex-col">
                    <p className="font-medium">
                      {job.name} ({job.yourBatch} {job.yourDepartment})
                    </p>
                    <p className="text-sm text-gray-400 -mt-1">
                      {job.yourCurrentRole} at {job.yourCurrentCompany}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-800 p-5 pb-8 rounded-xl mt-10">
                <h2 className="text-center mb-5 text-2xl font-semibold text-rose-500">
                  Review Jobs
                </h2>
                <form
                ...
                >
                  <div className="flex-1">
                    <Input
                      ...
                      reactHookForm={register("statusDesc", {
                        maxLength: {
                          value: 511,
                          message: "Max length is 511 characters",
                        },
                  ...
                  <button
                    ...
                    className="bg-[#4B164C] hover:bg-black disabled:bg-gray-600 h-10 self-end px-5 py-2 mt-2 rounded-lg cursor-pointer text-white"
                  >
                    Submit
                  </button>
                </form>
