...
const CreateAlumniProfile = () => {
    ...
    const { data: alumni, isLoading, isError, refetch } = useQuery({
        queryKey: ['66d769cc0004f4437921', user.email],
        queryFn: () => getAlumniProfile('66d769cc0004f4437921', user.email),
        enabled: !!user.email,
        retry: 2,
    })
    ...
    const onSubmit = useCallback(async (data) => {
        ...
        setLoading(true);
        try {
            const doesProfileExist = await getAlumniProfileWithUserName('66d769cc0004f4437921', data.username);
                ...
                !alumni ?
                    <form className='flex gap-3 flex-col' onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex md:flex-row flex-col gap-5 py-3">
                            ...
                            <Select
                                label='Last course attended at NITSGR?'
                                id='role'
                                require={true}
                                options={[
                                    {
                            ...
                            <Select
                                label='Highest Degree at NITSGR'
                                id='degree'
                                options={[
