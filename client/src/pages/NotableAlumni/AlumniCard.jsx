...
const AlumniCard = ({ alum }) => {
    const imageSrc = alum.image
        ? alum.image
        : `https://ui-avatars.com/api/?name=${encodeURIComponent(alum.name || 'Alumni')}&background=0D8ABC&color=fff&size=512`;

    return (
        <div data-aos="fade-in" className='rounded-3xl shadow-lg bg-[#205781] cursor-pointer bg-[linear-gradient(to_right,#4f4f4f2e_1px,transparent_1px),linear-gradient(to_bottom,#4f4f4f2e_1px,transparent_1px)] bg-[size:20px_24px] hover:bg-[#101010] hover:text-white border border-gray-900'>
            <div className='relative flex hover:scale-[98%] transition flex-col lg:items-start items-center justify-start lg:py-6 py-10 lg:gap-3 md:gap-3 gap-6 px-5 lg:max-w-full max-w-[22rem] m-auto'>
                <div className='relative lg:h-28 md:h-32 h-36 lg:w-28 md:w-32 w-36 rounded-full flex items-center justify-center lg:min-w-[6rem]'>
                    <img
                        loading='lazy'
                        className='lg:h-28 md:h-32 h-36 lg:w-28 md:w-32 w-36 rounded-full object-cover'
                        src={imageSrc}
